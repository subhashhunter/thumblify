...
const MyGeneration = () => {
  ...
  const handleDelete=async(id:string)=>{
    try {
      const confirm=window.confirm('Are you sure want to delte this thumbnail')
      if(!confirm) return
    ...
    <div className='mt-32 min-h-screen px-6 md:px-16 lg:px-24 xl:px-32'>
      ...
      {loading && (
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
          {Array.from({length:6}).map((_,i)=>(
            <div key={i} className='rounded-2xl bg-white/6 border border-white/10
      ...
      {!loading && thumbnails.length==0 && (
        <div className='text-center py-24'>
          <h3 className='text-lg font-semibold text-zinc-200'>No thumbnails yet
          </h3>
            ...
            return (
              ...
              border border-white/10 transition shadow-xl break-inside-avoid'>
                ...
                <div className={`relative overflow-hidden rounded-t-2xl ${aspectClass} bg-black`}>
                  ...
                  {thumb.isGenerating && <div className='absolute inset-0 bg-black/50 flex items-center
                 ...
                 <div className='p-4 space-y-2'>
                    ...
                    <div onClick={(e)=>e.stopPropagation()} className='absolute bottom-2 right-2 max-sm:flex sm:hidden
                    group-hover:flex gap-1.5'>
                      <TrashIcon
                      onClick={()=>handleDelete(thumb._id)}
                       className='size-6 bg-black/50 rounded hover:bg-pink-600 transition-all'/>
                      <Download
                      onClick={()=>handleDownload(thumb.image_url!)} className='size-6 bg-black/50 rounded hover:bg-pink-600 transition-all'/>
                      <Link target='_blank'
